...
func (c *bgpController) syncPeers(l log.Logger) error {
	...
	for _, p := range c.peers {
		...
		if p.session != nil && !shouldRun {
			...
			s, err := c.sessionManager.NewSession(c.logger, net.JoinHostPort(p.cfg.Addr.String(), strconv.Itoa(int(p.cfg.Port))), p.cfg.SrcAddr, p.cfg.MyASN, routerID, p.cfg.ASN, p.cfg.HoldTime, p.cfg.KeepaliveTime, p.cfg.Password, c.myNode, p.cfg.BFDProfile, p.cfg.MultiHop)
			if err != nil {
