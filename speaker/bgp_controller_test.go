...
func (f *fakeBGPSessionManager) NewSession(_ log.Logger, addr string, _ net.IP, _ uint32, _ net.IP, _ uint32, _ time.Duration, _ time.Duration, _, _, _ string, _ config.PeerMultiHop) (bgp.Session, error) {
	f.Lock()
	defer f.Unlock()
