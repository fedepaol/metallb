...
func TestBFDProfileNoSessions(t *testing.T) {
	...
	pp := map[string]*config.BFDProfile{
		"foo": {
			Name:                 "foo",
			EchoReceiveInterval:  uint32Ptr(60),
			EchoTransmitInterval: uint32Ptr(65),
...
func TestBFDWithSession(t *testing.T) {
	...
	pp := map[string]*config.BFDProfile{
		"foo": {
			...
			TransmitInterval:     uint32Ptr(70),
			DetectMultiplier:     uint32Ptr(5),
			EchoReceiveInterval:  uint32Ptr(90),
			EchoTransmitInterval: uint32Ptr(95),
			EchoMode:             false,
			PassiveMode:          false,
			MinimumTTL:           uint32Ptr(60),
		},
	...
	l := log.NewNopLogger()
	session, err := sessionManager.NewSession(l, "10.2.2.254:179", net.ParseIP("10.1.1.254"), 100, net.ParseIP("10.1.1.254"), 200, time.Second, "password", "hostname", "foo")
	if err != nil {
