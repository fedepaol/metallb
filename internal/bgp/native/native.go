...
func (sm *sessionManager) NewSession(l log.Logger, addr string, srcAddr net.IP, myASN uint32, routerID net.IP, asn uint32, holdTime, keepaliveTime time.Duration, password, myNode, bfdProfile string, multiHop config.PeerMultiHop) (bgp.Session, error) {
	ret := &session{
		addr:          addr,
		srcAddr:       srcAddr,
		myASN:         myASN,
