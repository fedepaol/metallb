...
func setupContainers(ipv4Addresses, ipv6Addresses []string) ([]*frrcontainer.FRR, error) {
	/*
		...
		Then we spin up a total of 3 containers:
		  * ibgp container that uses the first IP, a single-hop away from our speakers.
		  * ebgp container that uses the second IP, a single-hop away from our speakers,
		    and is connected to another containers network "multi-hop-net".
		  * another ebgp container connected to the "multi-hop-net", multi-hops away
		    from our speakers.
		We then wire these networks by adding static routes to both the speaker nodes
		containers (we're using kind) and the third (ebgp) container, using the second
		container as a gateway.
	*/
	...
	ebgpMultiHopContainerConfig := frrcontainer.Config{
		Name:        "ebgp-multi-hop",
		IPv4Address: "172.30.0.100",
		IPv6Address: "fc00:f853:ccd:e798::100",
		Neighbor: frrconfig.NeighborConfig{
			ASN:      MetalLBASN,
			Password: "ebgp-test",
			MultiHop: true,
	...
	if containersNetwork == "host" {
		...
		res, err = createFRRContainers(ibgpSingleHopContainerConfig, ebgpMultiHopContainerConfig, ebgpSingleHopContainerConfig)
		if err != nil {
			return res, err
		}
